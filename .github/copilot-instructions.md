...
- Stack: Astro 5 SSR (Vercel adapter), deployed on Vercel. Tailwind v4 + HeroUI + Membrane Palette design tokens (ex.: `var(--color-neutral-100)`, `var(--shadow-md)`, `var(--gradient-membrane)`). Tokens definidos em `src/styles/design-tokens.css`; spec em `design-system/`. Site em português (`lang="pt-br"`), datas em `pt-BR`.
